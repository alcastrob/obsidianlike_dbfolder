...
### Interactive `<%label%>` placeholders in a row template

A row template (`current_row_template`/`templatePath` — note-backed databases only, see above; `dbFolderPanel.ts` never overrides `getNewRowTemplate()` so folder-backed panels have no template support at all) can contain `<%label%>` placeholders — in the body content *and* in frontmatter values (including inside arrays, e.g. a `multiSelect`/`tags` list) — that get asked as a question before the row is created, instead of being written to disk literally. `src/core/templatePrompts.ts` (no `vscode` dependency, like the rest of `core/`) implements the parsing/substitution: `collectPlaceholderLabels()` walks the template's content + data and returns the distinct trimmed labels in first-appearance order — the label text *is* the identity, so two `<%Título%>` occurrences (body or frontmatter, doesn't matter) share one answer, asked only once — and `substitutePlaceholdersInText`/`substitutePlaceholdersInData` do the replacement afterward.

The actual prompting lives in `DatabaseHost`'s `"addRow"` case (`src/databaseHost.ts`), which has the `vscode` API: one sequential native `showInputBox()` per distinct label, awaited before the next is shown. Cancelling any one of them (Esc → `undefined`) aborts row creation entirely — no file is written — rather than leaving a note with a mix of substituted and literal `<%...%>` text; a subsequent "+ New" retries from scratch. This intentionally mirrors Obsidian Templater's per-value prompt convention but is this extension's own minimal syntax (no expression evaluation, no default values) — it is unrelated to the vault plugin's own "JS formulas" field, which this extension stores for file-format compatibility but doesn't execute (see `globalSettingsPanel.ts`'s `SECTIONS`).

One placeholder is auto-filled instead of asked about: `<%date%>` / `<%date FORMAT%>` (case-insensitive `date` keyword) is substituted with today's date via `substituteDatePlaceholdersInText`/`substituteDatePlaceholdersInData`, using `FORMAT`'s `YYYY`/`YY`/`MM`/`DD` tokens verbatim (anything else in `FORMAT` — slashes, dashes, literal words — passes through unchanged) and defaulting to `YYYYMMDD` when no `FORMAT` is given. `databaseHost.ts` runs this pass over both `templateContent` and `templateData` *before* `collectPlaceholderLabels()`, so a `<%date ...%>` placeholder is already gone by the time the interactive-prompt labels are collected — reorder those two calls and every date placeholder starts popping up as a question instead.
...
### Filter condition values commit on blur/Enter, not on every keystroke

Every `updateView`/`updateColumn` post triggers `DatabaseHost.sendSnapshot()` (`src/databaseHost.ts`) → `buildSnapshot()` → a full `resolveRows(this.config)` — a folder rescan, or in query mode a round-trip through `dataviewBridge.ts` re-running the `WHERE` query and rebuilding every matching row — plus a `persistConfig()` write, then ships the whole snapshot back down. That's fine for discrete actions (a select's `onChange`, a checkbox toggle, a button click), but `ConditionRow`'s filter-value `<input>` (`ToolbarMenus.tsx`) used to post on every `onChange`, so typing a multi-character value fired that whole round-trip per keystroke — with a query returning many rows this was slow enough to look like a hang. `ConditionValueInput` (same file) now keeps the typed value in local state and only calls `onCommit` (which posts `updateView`) on blur or Enter, the same commit-on-blur pattern `TableView.tsx`'s column-rename `<input>` already used. Any other free-text input wired straight to a live-recalculating `post()` should follow the same pattern rather than firing per keystroke.
